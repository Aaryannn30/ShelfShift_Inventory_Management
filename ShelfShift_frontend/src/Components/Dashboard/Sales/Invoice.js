...
const Invoice = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br dark:bg-[#030637]">
      {/* Header */}
      <header className="bg-white m-5 dark:bg-[#17153B] border shadow-lg p-4 flex justify-between items-center">
          ...
          {dropdownOpen && (
            <div className="absolute left-0 mt-2 w-48 bg-white border border-gray-300 rounded-lg shadow-lg z-10">
              {['All', 'Draft', 'Pending Approval', 'Approved', 'Confirmed', 'For Packaging', 'To be Shipped', 'Shipped', 'On Hold', 'New Custom View'].map((status, index) => (
                <a key={index} href="#" className="block px-4 py-2 text-blue-700 dark:text-blue-300 hover:bg-gray-100">
                  {status}
                </a>
              ))}
            </div>
      ...
      {/* Main Content */}
      <main className="p-6 bg-white m-5 dark:bg-[#17153B] border">
        <div className=" p-8 shadow-lg rounded-lg">
          ...
          <div className="mt-12">
            <h3 className="text-center text-xl font-semibold mb-6 text-blue-700 dark:text-blue-300">
              Life Cycle of an Invoice
            </h3>
